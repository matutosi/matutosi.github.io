...
まだちゃんとは使っていないが，動くようにはなった．
ちゃんと使うときは別のPCにインストールするので，そのためのメモ．
...
Tailscaleの管理画面での設定する．

- 「DNS」タブでMagicDNSを有効にする
- 「HTTPS Certificates」でHTTPSを有効にする

Tailscaleのドメイン名（例: <machine_name>.tail<xxxxxxx>.ts.net）の取得

<machine_name>と<xxxxxxx>は，tailscaleのMachinesとDNSに表示される内容と同じ．

Nextcloudの信頼できるドメイン設定とその他設定．
...
上の最後の行目は，/var/snap/nextcloud/current/nextcloud/config/config.php を編集するのと同じはず．
...
- https://www.youtube.com/watch?&v=jOYG10CvZZA&pp=ugUHEgVlbi1VUw%3D%3D
