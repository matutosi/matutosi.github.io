...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{importFrom}\NormalTok{(magrittr,}\StringTok{"\%\textgreater{}\%"}\NormalTok{)}
...
\NormalTok{wordApp }\OtherTok{\textless{}{-}} \FunctionTok{COMCreate}\NormalTok{(}\StringTok{"Word.Application"}\NormalTok{)}
\NormalTok{wordApp[[}\StringTok{"Visible"}\NormalTok{]] }\OtherTok{\textless{}{-}} \ConstantTok{TRUE}
\NormalTok{wordApp[[}\StringTok{"DisplayAlerts"}\NormalTok{]] }\OtherTok{\textless{}{-}} \ConstantTok{FALSE}
\NormalTok{path\_To\_PDF\_File }\OtherTok{\textless{}{-}} \StringTok{"xxx.pdf"}
\NormalTok{path\_To\_Word\_File }\OtherTok{\textless{}{-}} \StringTok{"xxx.docx"}
\NormalTok{doc }\OtherTok{\textless{}{-}} 
\NormalTok{  wordApp[[}\StringTok{"Documents"}\NormalTok{]]}\SpecialCharTok{$}\FunctionTok{Open}\NormalTok{(}\FunctionTok{normalizePath}\NormalTok{(path\_To\_PDF\_File), }
    \AttributeTok{ConfirmConversions =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{doc}\SpecialCharTok{$}\FunctionTok{SaveAs2}\NormalTok{(path\_To\_Word\_File)}
\end{Highlighting}
\end{Shaded}

\hypertarget{ux30e9ux30c3ux30d1ux30fcux95a2ux6570}{%
\subsubsection{ラッパー関数}\label{ux30e9ux30c3ux30d1ux30fcux95a2ux6570}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(RDCOMClient)}
\NormalTok{pdf2docx }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(pdf, }\AttributeTok{docx =} \ConstantTok{NULL}\NormalTok{)\{}
  \ControlFlowTok{if}\NormalTok{(}\FunctionTok{is.null}\NormalTok{(docx))\{}
\NormalTok{    docx }\OtherTok{\textless{}{-}} \FunctionTok{paste0}\NormalTok{(}\FunctionTok{getwd}\NormalTok{(), }\FunctionTok{sub}\NormalTok{(}\StringTok{"pdf"}\NormalTok{, }\StringTok{"docx"}\NormalTok{, pdf))}
\NormalTok{  \}}
\NormalTok{  wordApp }\OtherTok{\textless{}{-}}\NormalTok{ RDCOMClient}\SpecialCharTok{::}\FunctionTok{COMCreate}\NormalTok{(}\StringTok{"Word.Application"}\NormalTok{)}
\NormalTok{  wordApp[[}\StringTok{"Visible"}\NormalTok{]] }\OtherTok{\textless{}{-}} \ConstantTok{TRUE}
\NormalTok{  wordApp[[}\StringTok{"DisplayAlerts"}\NormalTok{]] }\OtherTok{\textless{}{-}} \ConstantTok{FALSE}
\NormalTok{  doc }\OtherTok{\textless{}{-}} 
\NormalTok{    wordApp[[}\StringTok{"Documents"}\NormalTok{]]}\SpecialCharTok{$}\FunctionTok{Open}\NormalTok{(}\FunctionTok{normalizePath}\NormalTok{(pdf), }\AttributeTok{ConfirmConversions =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{  doc}\SpecialCharTok{$}\FunctionTok{SaveAs2}\NormalTok{(docx)}
\NormalTok{  doc}\SpecialCharTok{$}\FunctionTok{close}\NormalTok{()}
\NormalTok{\}}
